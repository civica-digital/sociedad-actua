INVESTOR_CAUSES = [
  'Accesibilidad, movilidad y vialidad',
  'Adicciones',
  'Alimentación, horticultura y agricultura',
  'Animales',
  'Arte, historia y cultura en general',
  'Asuntos internacionales',
  'Atención a víctimas de violencia',
  'Ciencias naturales y tecnología',
  'Ciencias sociales',
  'Comunicación',
  'Cultura de la legalidad',
  'Cultura de paz',
  'Cultura vecinal',
  'Deporte y recreación',
  'Derechos humanos y civiles',
  'Desarrollo comunitario y económico',
  'Discapacidad',
  'Educación',
  'Empleo',
  'Espacios públicos, planeación y desarrollo urbano',
  'Espiritualidad y religión',
  'Filantropía y voluntariado',
  'Fortalecimiento a OSCs',
  'Género y sexualidad',
  'Gobierno, administración pública y servicios públicos',
  'Investigación',
  'Jóvenes',
  'Jurídicos',
  'Medio ambiente',
  'Migrantes',
  'Niños(as)',
  'Participación ciudadana y actividades cívicas',
  'Prevención de la violencia',
  'Salud y emergencias médicas',
  'Seguridad y prevención del delito',
  'Tercera edad',
  'Vivienda y albergues',
  'Otro'
]
